...
- `POST /api/v1/auth/register` — створення активного студентського профілю та отримання JWT;
- `GET /api/v1/users/me` — профіль поточного користувача;
- `PUT /api/v1/users/me/password` — перевірена зміна пароля поточного користувача;
...
Реєстрація приймає логін із 5–15 літер або цифр, ім’я та прізвище до 20 символів і пароль із
8–128 символів без пробілів. Пароль одразу зберігається як PBKDF2-HMAC-SHA256 hash. Після успішної
зміни пароля клієнт має видалити поточний JWT і виконати повторний вхід.
