...
`/auth/refresh` — єдине місце, де акаунт перечитується з бази, тому саме там обриваються сесії.
Обмін відхиляється, якщо акаунт заблоковано чи видалено, або якщо пароль змінився після випуску
токена: `PUT /api/v1/users/me/password` оновлює `users.credentials_changed_at`, кожен токен несе
це значення в claim `cca`, і токен із попереднім значенням більше не подовжується. Вже виданий токен працює до кінця свого терміну
(до 15 хвилин) — сервер не перечитує акаунт на кожен запит; зміна пароля обмежує викрадену сесію
одним терміном дії токена замість необмеженого ланцюжка оновлень. Це стосується й того, хто
змінив пароль: його поточна сесія завершиться на наступному оновленні.
